...
if submitted:
    # Additional section for visualizing predictions over time for different types of surgeries
    st.title('Weight Prediction Over Time for Different Types of Surgeries')
    
    timestamps = np.arange(0, 3601, 100)  # From 0 to 3600 in steps of 100
    surgery_types = [5, 6, 8]
    ...
    for surgery in surgery_types:
        ...
        for time in timestamps:
            data = {
                ...
                'SurgeryTypeName': [surgery]
            }
            df_predict = pd.DataFrame(data)
            predicted_weight = model.predict(df_predict)
            
            if np.isscalar(predicted_weight):
                weight = predicted_weight
            elif len(predicted_weight.shape) == 1:
                weight = predicted_weight[0]
            else:
                weight = predicted_weight[0, 0]
            
            weights.append(weight)
        
        results[f'Surgery_{surgery}'] = weights
    
    # Plot the results
    plt.figure(figsize=(12, 8))
    for col in results.columns:
        plt.plot(timestamps, results[col], label=f'Surgery Type {col.split("_")[1]}')
    
    plt.title('Predicted Weight After Different Types of Surgeries')
